...
\begin{itemize}
    ...
    \item \codepath{generate_partitioned_dataset_bin.py} e
    \codepath{generate_prefix_constant_rho_dataset_bin.py}: generazione deterministica
...
\begin{enumerate}
    \item vengono prima generati i dataset, chiamati \codepath{dataset_n_{n}_d_{d}_p_{p}_s_{seed}.bin}, con gli script Python
    \codepath{scripts/generate_partitioned_dataset_bin.py} oppure
...
Nel progetto sono presenti due generatori:
\begin{itemize}
    \item \codepath{generate_partitioned_dataset_bin.py}, usato per dataset uniformi generali;
    \item \codepath{generate_prefix_constant_rho_dataset_bin.py}, usato per la campagna
    \texttt{prefix\_constant\_rho} del Capitolo~\ref{chp:risultati}.
\end{itemize}

Entrambi accettano i parametri \texttt{--n}, \texttt{--d}, \texttt{--p}, \texttt{--seed},
\texttt{--workers}, \texttt{--progress-batch} e producono file con nomenclatura deterministica:
\codepath{dataset_n_{n}_d_{d}_p_{p}_s_{seed}.bin}.

\subsection{Modalità \texttt{prefix\_constant\_rho}}

Lo script \codepath{generate_prefix_constant_rho_dataset_bin.py} introduce il vincolo:
\[
n \bmod d = 0,
\]
in modo da fissare
...
La costruzione è:
\begin{enumerate}
    \item estrazione di \(d\) ID distinti nel dominio \([0,\min(n,2^{32})-1]\);
    \item emissione della prima occorrenza di un nuovo ID all'inizio di ogni blocco;
    \item completamento delle altre \(\rho-1\) posizioni campionando dagli ID già visti;
...
\begin{enumerate}
    ...
    \codepath{generate_partitioned_dataset_bin.py};
