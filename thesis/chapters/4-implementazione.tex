...
Il formato è implementato in \texttt{scripts/generate\_partitioned\_dataset\_bin.py}
(generazione) e \texttt{src/satp/io/BinaryDatasetIO.h} (parsing e I/O).
...
\begin{itemize}
    ...
    \item codifiche payload: \texttt{ENCODING\_ZLIB\_U32\_LE} per i valori e
    \texttt{ENCODING\_ZLIB\_BITSET\_LE} per il bitset.
\end{itemize}
...
\texttt{src/satp/io/BinaryDatasetIO.h} mantiene aperto il descrittore del file
...
Lo script \texttt{scripts/generate\_partitioned\_dataset\_bin.py} genera file con
nome:
\[
...
La generazione supporta parallelismo multiprocesso (parametro
\texttt{--workers}) e barra di avanzamento su \texttt{stderr}. La correttezza
strutturale è verificata nei test Python in
\texttt{scripts/tests/test\_generate\_dataset.py}.

\section{Interfacce comuni e hashing}

\subsection{Interfaccia base degli algoritmi}

Il contratto comune è definito in \texttt{src/satp/algorithms/Algorithm.h}:
\begin{itemize}
...
La randomizzazione è centralizzata in \texttt{src/satp/hashing.h}:
\begin{itemize}
...
\begin{center}
\small
\texttt{algorithm, params, mode, runs, sample\_size, element\_index, distinct\_count, seed,}\
\texttt{f0\_mean, f0\_hat\_mean, mean, variance, stddev, rse\_theoretical, rse\_observed,}\
\texttt{bias, difference, bias\_relative, mean\_relative\_error, rmse, mae}
\end{center}
...
I risultati sono salvati automaticamente in
\texttt{results/<Algorithm>/<params>/results\_oneshot.csv} (comando
\texttt{run}) oppure
\texttt{results/<Algorithm>/<params>/results\_streaming.csv} (comando
\texttt{runstream}), senza richiedere l'inserimento manuale del percorso.

\subsection{Orchestrazione batch}

Lo script \texttt{scripts/orchestrate\_benchmarks.py} automatizza:
\begin{itemize}
...
\begin{itemize}
    ...
    (\texttt{variance}, \texttt{stddev}, \texttt{rmse}, \texttt{mae},
    \texttt{mean\_relative\_error}, \texttt{difference}) siano non negative;
...
\texttt{scripts/tests/test\_generate\_dataset.py} valida:
\begin{itemize}
