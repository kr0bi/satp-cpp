...
\begin{enumerate}
    ...
    \codepath{results/<namespace>/<mode>/<algorithm>/<params>/results_*.csv}
...
necessario all'esecuzione corrente tramite \codepath{BinaryDatasetPartitionReader}
...
\subsection{Modalità normal, streaming, merge}
...
\subsection{Post-processing: analisi Type A e Type B}

Dal medesimo CSV di \texttt{streaming} vengono costruite due viste analitiche nel notebook:
\begin{itemize}
    \item \textbf{Type A}: ascissa \(F_0(t)\) reale (\texttt{f0\_mean\_t}),
    ordinata \(\hat{F}_0(t)\) media (\texttt{f0\_hat\_mean\_t}), pannelli per
    valori di \(\rho=n/d\).
    \item \textbf{Type B}: fissato un target \(F_0^{\ast}\), ascissa
    \(\rho_t=t/F_0(t)\) (ripetizioni medie effettive), ordinata
    \(\hat{F}_0\) al target. I punti sono ricavati per interpolazione lineare
    sui checkpoint disponibili.
\end{itemize}

Nei pannelli in scala log-log, la banda di dispersione è trattata in forma
moltiplicativa (coerente con la scala logaritmica), partendo da media \(\mu\)
...
\]
Nei pannelli lineari resta la banda classica \(\mu \pm \sigma\).
...
\begin{itemize}
    ...
    \item \texttt{mode}: modalità di esecuzione (\texttt{normal}, \texttt{streaming}, \texttt{merge}).
...
L'entry point è il \codepath{main.cpp} che istanzia \codepath{BenchmarkCli}. I comandi sono:
...
\begin{itemize}
    ...
    \item \texttt{list}: elenca gli algoritmi disponibili (\texttt{hllpp}, \texttt{hll}, \texttt{ll}, \texttt{pc}).
    \item \texttt{set <param> <value>}: imposta un parametro della sessione. I parametri supportati sono \texttt{datasetPath}, \texttt{resultsNamespace}, \texttt{k}, \texttt{l}, \texttt{lLog}.
    \item \texttt{run <algo|all>}: esegue uno o più algoritmi in modalità \texttt{normal} (metriche all'endpoint \(t=n\)).
...
Per i comandi \texttt{run}, \texttt{runstream} e \texttt{runmerge}, l'argomento
\texttt{<algo>} può essere uno tra \texttt{hllpp}, \texttt{hll}, \texttt{ll},
\texttt{pc}, oppure \texttt{all}.

Il metodo \codepath{PathUtils::buildResultCsvPath} costruisce i percorsi:
\begin{itemize}
    \item \codepath{results/<namespace>/oneshot/<AlgorithmName>/<params>/results_oneshot.csv};
    \item \codepath{results/<namespace>/streaming/<AlgorithmName>/<params>/results_streaming.csv};
    \item \codepath{results/<namespace>/merge/<AlgorithmName>/<params>/results_merge.csv}.
\end{itemize}
...
\begin{enumerate}
    ...
    \item il metodo \codepath{indexBinaryDataset} legge il campo e lo valida nel dominio
    \texttt{uint32};
    \item il metodo \codepath{CliConfig::loadDatasetRuntimeContext} trasferisce il 
    \texttt{seed} nel contesto runtime \texttt{DatasetRuntimeContext};
    \item il \codepath{EvaluationFramework} copia \texttt{binaryDataset.info.seed}
...
\begin{itemize}
    ...
    \item le tre modalita' di run (\texttt{normal}, \texttt{streaming}, \texttt{merge});
