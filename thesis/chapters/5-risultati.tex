...
I parametri individuati sono riportati nella Tabella~\ref{tab:configurazione-ottimale-selection}.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|l|c|r|}
        ...
        \hline
    \end{tabular}
    \caption{Configurazione ottimale per algoritmo nel dominio esplorato.}
    \label{tab:configurazione-ottimale-selection}
\end{table}
...
ottimale della Tabella~\ref{tab:configurazione-ottimale-selection} e confrontando i seed
...
\begin{figure}[htbp]
    ...
    \caption{Distribuzione della MRE sui seed nelle configurazioni ottimali.}
...
ma con MRE assoluta elevata (circa \(0.5\) nella configurazione ottimale), ricordando che ``stabile'' non implica ``accurato''.
...
\begin{figure}[htbp]
    ...
    \caption{Configurazione ottimale: confronto tra stima media \(\hat{F}_0(t)\) e valore reale \(F_0(t)\) in scala lineare (seed 21041998).}
...
\begin{figure}[htbp]
    ...
    \caption{Configurazione ottimale: varianza della stima in modalità streaming (seed 21041998, scala log-log).}
...
ottimali perché nel dataset usato vale \(n=d\) e l'endpoint è deterministico
tra run.

\subsection{Metriche aggregate in modalità \textit{normal}}

La Tabella~\ref{tab:best-stream-summary} riporta le metriche associate alla modalità di esecuzione
\textit{normal} mediate sui seed per ciascun valore finale di \(F_0\).
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|l|r|r|r|r|r|}
        ...
        \hline
    \end{tabular}
    \caption{Configurazione ottimale: metriche endpoint aggregate in modalità streaming (media sui seed).}
    \label{tab:best-stream-summary}
\end{table}

La Figura~\ref{fig:best-stream-mre-by-f0} visualizza lo stesso confronto sull'unica metrica MRE.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.90\textwidth]{figures/results/best_stream_final_mre_by_algorithm_and_f0.png}
    \caption{Configurazione ottimale: MRE endpoint per algoritmo e valore finale di \(F_0\) (media sui seed).}
    \label{fig:best-stream-mre-by-f0}
\end{figure}


\section{Risultati merge con la configurazione ottimale}
...
\begin{figure}[htbp]
    ...
    \caption{Configurazione ottimale: distribuzione di \codepath{delta\_merge\_serial\_abs}.}
...
\begin{figure}[htbp]
    ...
    \caption{Configurazione ottimale: conteggio dei casi con delta non nullo nel confronto merge vs seriale.}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|l|r|r|r|r|r|}
        ...
        \hline
    \end{tabular}
    \caption{Configurazione ottimale: confronto merge vs seriale. Tutti i delta risultano nulli nel dataset analizzato.}
