...
La prima analisi eterogenea riguarda il confronto tra due sketch che utilizzano
funzioni di hash differenti.
La configurazione considerata \`e stata:
\[
n=10^7,\qquad p=50,\qquad seed=21041998,\qquad
d\in\{10^5,10^6,10^7\}
\]
con precisione fissata a \(k=14\), da $n$ e $d$, si ricava $\rho\in\{100,10,1\}$. 

La configurazione \texttt{hash mismatch} include anche uno scenario \texttt{ctrl}
  omogeneo, usato come riferimento locale nella stessa figura e nella stessa
  tabella. Questo controllo non costituisce un esperimento autonomo distinto dal
  caso omogeneo discusso in precedenza, ma ne rappresenta un sottoinsieme con
  HyperLogLog++ e \(k=14\), utile per confrontare direttamente i casi
  \texttt{invalid} con la baseline corretta.

Il controllo omogeneo è eseguito con
strategia \texttt{direct}, mentre i casi \texttt{invalid} sono analizzati con
\texttt{reject} e \texttt{unsafe\_naive\_merge}.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[t]{0.78\textwidth}
        ...
        \caption{Errore relativo medio del merge forzato per scenario e \(\rho\)}
...
La figura mette in evidenza il contrasto tra il controllo omogeneo e il merge
forzato nei casi \texttt{invalid}; la tabella seguente ne riassume i valori
numerici principali. Le quantità \(\overline{\varepsilon}_{merge}\) e
...
disallineamento di seed all'interno della stessa famiglia di hash. Le etichette
\texttt{family} e \texttt{family\_rev} indicano invece il confronto tra
famiglie diverse nelle due direzioni. Nella matrice minima, il disallineamento
del seed è stato testato sulle sole famiglie \texttt{xxhash64} e
\texttt{murmurhash3}.
...
Nei casi \texttt{invalid}, il merge forzato mostra un degrado molto marcato e
sistematico rispetto al riferimento seriale.
Per \(\rho=1\) l'errore relativo medio raggiunge valori prossimi o superiori a
1, con massimo 1.009634.
La strategia \texttt{reject} non produce una stima di merge per le coppie
incompatibili, ma consente di registrare esplicitamente il caso nel protocollo
sperimentale, mantenendo disponibili il riferimento seriale e l'unione esatta.
