...
In seguito viene descritta l'implementazione del sistema sperimentale sviluppato
per confrontare algoritmi di stima della cardinalità sui flussi di dati.

Lo studio degli algoritmi avviene in due modalità, una che simula la ricezione dei dati come 
in un servizio di streaming e l'altra che analizza come i varie sketch prodotti dagli algoritmi
si comportino alla loro unione.

Il capitolo è costruito
attorno a tre nuclei principali:
\begin{itemize}
    \item gli algoritmi, la loro interfaccia comune e le scelte adottate per
    la stesura in codice delle strutture descritte nella letteratura;
    \item le funzioni di hash, il loro ruolo nella semantica degli \emph{sketch} e la
...
I principali componenti del sistema sono stati implementati in \texttt{C++}, mentre in \texttt{Python}
sono stati sviluppati gli script di generazione del dataset usato e di orchestrazione degli esperimenti.
...
\begin{figure}[htbp]
    \centering
    \resizebox{0.98\textwidth}{!}{\input{figures/system_architecture}}
    \caption{Architettura del sistema e principali dipendenze tra moduli e artefatti. Le frecce indicano le relazioni di dipendenza oppure il verso del flusso dei dati}
    \label{fig:system-architecture}
\end{figure}

Dal punto di vista operativo, la catena di esecuzione completa è la seguente:
\begin{enumerate}
    \item il dataset viene generato in \texttt{Python} e serializzato
...
Questa sezione raccoglie le scelte adottate per tradurre in codice gli algoritmi
considerati nei capitoli precedenti. L'obiettivo è mostrare come siano stati
resi confrontabili all'interno di un'unica infrastruttura, pur mantenendo le
specificità delle singole implementazioni.
...
Prima dell'implementazione vera e propria degli algoritmi discussi precedentemente, è presente anche
un'implementazione basica, che è stata inizialmente utilizzata come riferimento per gli altri algoritmi.

Essa è \texttt{NaiveCounting}. Questa classe non viene esposta come algoritmo degli esperimenti
principali, ma è essenziale nei test del framework perché fornisce un
riferimento privo di errore di stima.

Il primo algoritmo vero e proprio implementato è \texttt{ProbabilisticCounting}, che deriva dal lavoro di
...
algoritmi probabilistici per il conteggio degli elementi distinti.

Vengono poi a seguire \texttt{LogLog} e \texttt{HyperLogLog}, che
...
per i confronti in modalità di flusso sia il punto di partenza per lo studio del comportamento dell'unione tra sketch in casi non ottimali.
...
Ogni algoritmo ha dei limiti derivati dagli articoli di riferimento, nei parametri che utilizza 
come, ad esempio, la dimensione dei registri o la lunghezza della bitmap.

I limiti degli algoritmi non sono lasciati a vincoli
...
\paragraph{Algoritmo esatto}
\texttt{NaiveCounting} mantiene un insieme ordinato di identificatori distinti
tramite \texttt{uint32}. Il metodo \texttt{process} inserisce un
...
\paragraph{LogLog}
L'implementazione usa un vettore di registri \texttt{uint8\_t} e mantiene in modo
...
serializzato uniformemente come \texttt{k} per mantenere un formato dei
...
Le funzioni di hash costituiscono un livello trasversale dell'intera
implementazione, perché incidono sia sul funzionamento degli sketch sia sulla
correttezza dei confronti sperimentali. Per questo motivo vengono discusse a
parte, mettendo in evidenza il loro ruolo, l'interfaccia comune e i criteri di
selezione adottati.

\subsection{Ruolo dell'hash negli sketch}

Negli algoritmi implementati l'hash non serve solo a ottenere un valore di hash uniforme. 
Esso è parte integrante della funzionalit\`a degli algoritmi, per due principali ragioni: per poter
scegliere in maniera libera quale funzione di hash utilizzare e per poter sperimentare 
il merge eterogeneo in caso di funzioni di hash differenti.
...
processo di merge corretto non dipende soltanto dal tipo dell'algoritmo, ma anche dalla
...
\codepath{HashFactory.cpp}. Il sistema, attualmente, supporta quattro funzioni di hash:
...
Il seed entra nel sistema in due punti distinti. Il primo luogo in cui viene definito è il seed del dataset,
che viene memorizzato nell'header binario e propagato nel contesto durante il tempo di esecuzione. Il
...
ha come valore di default il seed del dataset. Nel merge eterogeneo, invece, i campi
\texttt{leftHashSeed} e \texttt{rightHashSeed} prevalgono quando sono
valorizzati, mentre in assenza di override resta attivo il valore di default al seed del
dataset.

\section{Framework sperimentale}

Dopo avere definito algoritmi e hashing, resta da descrivere il livello che
coordina l'esecuzione degli esperimenti. In questa sezione vengono quindi
presentati i requisiti del framework, la sua organizzazione interna e i flussi
con cui vengono prodotti i risultati.
...
\begin{itemize}
    ...
    prefissi;
    \item una modalità di merge omogenea, per confrontare merge e riferimento
    seriale;
    \item una modalità di merge eterogenea, per descrivere casi validi,
    recuperabili o non validi e misurarne il comportamento.
\end{itemize}

La distinzione tra le due modalità di merge era necessaria, per verificare che l'intero sistema
si comportasse in maniera accurata nel caso di merge con parametri sbagliati.
...
\begin{itemize}
    \item \codepath{Cli} come interfaccia interattiva verso
    l'utente;
    ...
    \item \codepath{EvaluationFramework}, riesportato da
...
costruisce piuttosto oggetti del tipo \texttt{AlgorithmJob} tramite
...
\codepath{buildResultCsvPath(...)} secondo lo schema generale
\begin{center}
\small
...
Questa decisione \`e stata presa con l'obiettivo di non registrare solamente l'unione di due sketch
ma \`e necessaria per riprodurre correttamente quale fosse il contesto dell'unione.
...
sono l'unica sorgente autorevole della configurazione sperimentale.

La terza è la separazione tra valutazione e serializzazione. Le modalità del
framework producono strutture dati in memoria; lo strato architetturale della parte da riga di comando si
...
Il sistema usa un solo protocollo di generazione dei dati, chiamato
\texttt{prefix\_constant\_rho}. 

In questa sezione non entreremo in dettaglio, poich\'e verr\`a trattato
nel capitolo \ref{chp:risultati_nuovo}.
Ma resta necessaria perché la misurazione dei risultati dipende direttamente da alcune
proprietà del dataset, in particolare dalla struttura in cui \`e caratterizzato il dataset.
...
\begin{itemize}
    ...
    \item il numero di elementi distinti per partizione indicato con \(d\) nel file;
...
partizione alla volta tramite \codepath{PartitionReader}. Il
...
dover ricostruire ogni volta un insieme esplicito dei valori distinti visti fino al
punto corrente.

\subsection{Protocollo \texttt{prefix\_constant\_rho}}
\label{sec:prefix-constant-rho}

I dataset utilizzati sono stati generati con uno script
\codepath{generate_prefix_constant_rho_dataset_bin.py}. Il generatore
impone il vincolo che:
\[
n \bmod d = 0
\]
...
Poiché ogni blocco di lunghezza $\rho$ introduce esattamente una nuova prima
  occorrenza, al termine del $j$-esimo blocco il prefisso contiene esattamente
  $j$ elementi distinti:
  \[
  F_0(j \cdot \rho)=j
  \]

Ricordiamo che $F_0(t)$ \`e il numero di elementi distinti osservati nel prefisso di lunghezza $t$.
...
\begin{itemize}
    \item il seed globale è descritto nell'header del file;
    \item ogni partizione usa un seed derivato in modo deterministico tramite una funzione nel framework;
    \item il generatore è testato esplicitamente con dei test di unit\`a in
    \codepath{test_generate_prefix_constant_rho_dataset.py}.
\end{itemize}
...
Il file \codepath{simulation/EvaluationFrameworkTest.cpp} controlla che:
\begin{itemize}
    ...
    \item il pianificatore dei punti di controllo copra correttamente il flusso e rispetti la configurazione, cio\`e che avvengano esattamente un certo numero di checkpoint;
...
aggiunta, gli script di orchestrazione costruiscono i valori dei comandi per la
riga di comando in modo deterministico, fissando il nome dei file risultanti e del loro percorso,
