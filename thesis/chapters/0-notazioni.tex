...
\begin{itemize}
    ...
    \item $\widehat{\mathrm{Bias}}(\hat{\theta})$: stima empirica del bias.
    \item $\mathrm{RB}$: relative bias.
    \item $\mathrm{MRE}$: mean relative error.
    \item $\mathrm{AB}$: absolute bias.
    \item $(\Omega,\mathcal{F},\mathbb{P})$: spazio di probabilità.
    \item $\mathcal{F}$: $\sigma$-algebra su $\Omega$.
    \item $(\mathcal{X},\mathcal{A})$: spazio misurabile dei valori.
\end{itemize}
