...
$\mathcal{U} \subseteq \mathbb{N}$. Uno \emph{stream di dati} è una sequenza ordinata
di elementi
\[
...
Dato uno stream $S$, la \emph{frequenza} di un elemento $a \in \mathcal{U}$ è
\[
f(a) = |\{ i \mid x_i = a \}|.
\]
...
Il \emph{numero di distinti} nello stream $S$ è
\[
...
Lo stream è una sequenza di aggiornamenti del tipo $(a_t, \Delta_t)$, con
...
\end{definition}
\paragraph{Esempio.}
Se lo stream è una lista di valori, ogni elemento $x_i$ può essere visto come
un aggiornamento $(x_i,1)$.

\begin{definition}[Modello \emph{turnstile}]
Nel modello \emph{turnstile} sono ammessi aggiornamenti con $\Delta_t$ anche
negativi, così che le frequenze possano aumentare o diminuire.
\end{definition}
\paragraph{Esempio.}
Un aggiornamento $(a_t,-1)$ decrementa la frequenza dell'elemento $a_t$.

\begin{definition}[Modello \emph{sliding window}]
Nel modello a \emph{sliding window} si considerano solo gli ultimi $W$
aggiornamenti della stream, e gli elementi più vecchi vengono dimenticati.
\end{definition}
\paragraph{Esempio.}
Con $W=10^6$, le statistiche sono calcolate solo sugli ultimi un milione di elementi.

I modelli \emph{turnstile} e \emph{sliding window} esulano dallo scopo di questa tesi.
...
In pratica si richiede che $m$ dipenda in modo polilogaritmico da $n$ e
