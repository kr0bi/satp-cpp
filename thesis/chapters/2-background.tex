...
Nel modello di stream i dati \cite{Leskovec_Rajaraman_Ullman_2014} arrivano in modo continuo; la stream pu\`o essere
potenzialmente infinita. Rispetto all'utilizzo di un database tradizionale, non \`e possibile
...
Poich\'e in streaming non possiamo calcolare esattamente $F_0$ mantenendo
memoria sublineare, adottiamo misure di accuratezza probabilistiche.
...
\begin{itemize}
    ...
    \item produrre una stima $\hat{F}_0$ con errore controllato, utilizzando $m$ bit, dove $m \ll n$.
\end{itemize}
...
La Figura~\ref{fig:hash-uniformity} mostra in modo intuitivo la mappatura
chiavi$\to$bucket realizzata da una funzione di hash.
\begin{figure}[htbp]
    \centering
...
\end{definition}
Nel caso specifico del count-distinct, la concatenazione induce la stessa
