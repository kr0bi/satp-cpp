...
stimatori del Capitolo~2.5.  Vengono fatte $R$ esecuzioni indipendentim, chiamate \textit{run}, con
...
\]
L'\emph{errore relativo medio} (Mean Relative Error, MRE) è
\[
\mathrm{MRE}=\frac{1}{R}\sum_{r=1}^{R}\frac{|\hat{F}_0^{(r)}-F_0^{(r)}|}{F_0^{(r)}},
\]
e le metriche aggregate usate nei risultati includono il
Mean Absolute Error (MAE) e il Root Mean Squared Error (RMSE):
\[
...
Senza ulteriori spiegazioni, queste quantità sono stime empiriche dei corrispondenti concetti teorici presentati precedentemente.
...
Analizzando algoritmi di streaming per il count-distinct, \`e importante notare che tipo di rapporto c'\`e tra
spazio e accuratezza. Non concentrandosi concretemente sull'algoritmo utilizzato, ci sono dei limiti
teorici che possiamo notare.
...
Mentre, ridurre $\delta$ richiede invece solo un fattore logaritmico, ottenibile
tramite ripetizioni indipendenti e combinazione (ad esempio \emph{median trick} \cite{Motwani_Raghavan_1996}).

Questa differenza sul trattare $\varepsilon$ e $\delta$ spiega perché in ricerca si investe soprattutto nella
riduzione di $\varepsilon$ e si regola $\delta$ con pochi esperimenti aggiuntivi.
