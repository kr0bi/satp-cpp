...
stimatori del Capitolo~2.5. Vengono fatte $R$ esecuzioni indipendenti, chiamate \textit{run}, con
...
\[
Var(\hat{F}_0) = \frac{1}{R-1}\sum_{r=1}^{R}\bigl(\hat{F}_0^{(r)}-\bar{\hat{F}}_0\bigr)^2,
\qquad
\sigma=\sqrt{Var}.
\]
...
\[
\mathrm{RSE}_{\text{obs}}=\frac{\sigma}{\bar{F}_0}.
\]
Quando il valore vero è lo stesso in tutte le run ($F_0^{(r)}\equiv F_0$),
questa definizione si riduce a $\sigma/F_0$. Quando disponibile, si
