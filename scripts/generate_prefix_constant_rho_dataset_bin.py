...
# Binary format v2 (same as the existing generator).
MAGIC = b"SATPDBN2"
VERSION = 2
...
def _validate_params(n: int, d: int, p: int) -> None:
    ...
    if n % d != 0:
        raise ValueError("constant-rho protocol requires n % d == 0")
...
def _generate_partition_values_constant_rho(rng: random.Random,
                                            n_per_partition: int,
                                            d_per_partition: int,
                                            id_domain_size: int) -> List[int]:
    rho = n_per_partition // d_per_partition

    # Distinct keys are random, but first occurrences follow a deterministic
    # block schedule: one new key every rho positions.
    distinct_ids = rng.sample(range(id_domain_size), d_per_partition)
    rng.shuffle(distinct_ids)

    values: List[int] = []
    seen: List[int] = []
    for new_id in distinct_ids:
        values.append(new_id)
        seen.append(new_id)
        for _ in range(rho - 1):
            values.append(seen[rng.randrange(len(seen))])

    if len(values) != n_per_partition:
        raise RuntimeError(
            f"Internal error: generated {len(values)} values, expected {n_per_partition}"
        )
    return values


def _build_truth_bits(values: List[int]) -> Tuple[bytes, int]:
    truth_byte = 0
    truth_bit_pos = 0
    ...
    return bytes(truth_chunk), distinct_seen
...
def _write_partition_fragment(task: Tuple[int, int, int, int, int, str, Any, int]) -> Tuple[int, str, str]:
    ...
    values = _generate_partition_values_constant_rho(
        rng=rng,
        n_per_partition=n_per_partition,
        d_per_partition=d_per_partition,
        id_domain_size=id_domain_size,
    )
    ...
    with values_path.open("wb", buffering=1024 * 1024) as out_values:
        ...
        if chunk:
            ...
            if compressed_chunk:
                out_values.write(compressed_chunk)
            del chunk[:]
        out_values.write(compressor.flush())

    truth_raw, distinct_seen = _build_truth_bits(values)
    if distinct_seen != d_per_partition:
        raise RuntimeError(
            f"Partition {part_idx}: generated distinct={distinct_seen} expected={d_per_partition}"
        )

    with truth_path.open("wb", buffering=512 * 1024) as out_truth:
        out_truth.write(zlib.compress(truth_raw, level=6))
...
def generate_prefix_constant_rho_dataset_bin(output_dir: pathlib.Path,
                                             n: int,
                                             d: int,
                                             p: int,
                                             seed: int,
    ...
    with tempfile.TemporaryDirectory(prefix="satp_prefix_rho_parts_", dir=str(output_dir)) as tmp_dir:
        id_domain_size = min(n, UINT32_DOMAIN_SIZE)
        ...
        if workers_eff == 1:
            ...
            with ctx.Manager() as manager:
                ...
                with ctx.Pool(processes=workers_eff) as pool:
                    ...
                    while True:
                        produced = _drain_progress_queue(
                            progress_queue, produced, total_elements, start, show_progress)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate one binary dataset with constant-rho prefix protocol "
            "(n % d == 0, same binary format v2)."
        )
    )
    parser.add_argument("--output-dir", type=pathlib.Path,
                        default=pathlib.Path("datasets/prefix_constant_rho"),
